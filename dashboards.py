...
def compare(fu, methods, reference_category=None, sharex=True, cols=2, func_unit="kg"):
    ...
    def graph_method_ref():
        with warnings.catch_warnings():
            ...
            #add a suptitle
            fig.suptitle("Comparison of different LCA on " + bd.Method(reference_category).name[1], fontsize=30, fontweight='bold', ha='center') #centered title in bold
            # add a subtitle
    ...
    def contributions(act, method):
        with warnings.catch_warnings():
            ...
            for c in df_contrib.index:
                names.append(bd.get_activity(code=c)['name'])
...
def impact_transfer(fu, methods, reference_category=None,limit=5, cols=3, func_unit="kg"):
    ...
    def heatmap():
        with warnings.catch_warnings():
            ...
            axes.set_xticks(range(len(df_norm.columns)), ['\n'.join(textwrap.wrap(label, 20)) for label in df_norm.columns],rotation=0,fontsize=20,ha="left")
            fig.suptitle("Comparison of different LCA", fontsize=20, fontweight='bold',x=0.44, y=1.02) #centered title in bold
            axes.set_title("for 1 " + func_unit,fontsize=17, ha='center',y=1.1, color='gray')
            plt.show()
        ...
        with warnings.catch_warnings():
            ...
            with warnings.catch_warnings():
                ...
                for ax, m in zip(axes, methods):
                    ...
                    ax.set_xticks([])
                    ax.set_yticks(range(len(result_df)), ['\n'.join(textwrap.wrap(label, 40)) for label in result_df['Activities']],fontsize=15)
                    #add each score of components
                    for bar in ax.patches:
                        ax.text(
                            bar.get_x() + 1.01 * bar.get_width(),
